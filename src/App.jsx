...
export default function MindflowTracker() {
  ...
  const startVoice = useCallback(async () => {
    const key = getApiKey();
    if (!key) { showToast("Сначала добавьте API ключ в настройках", "err"); return; }
    try {
      ...
      recorder.start();
      setIsRecording(true);
      setRecStatus("Говорите… (отпустите чтобы остановить)");
    } catch {
      setRecStatus("Нет доступа к микрофону — разрешите в настройках браузера");
    }
  }, []);
  ...
  const transcribeAudio = async (audioBlob) => {
    ...
    try {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: [
            { type: "text", text: "Transcribe this audio recording exactly as spoken. Return ONLY the transcribed text, nothing else. The speaker is Russian, transcribe in Russian." },
      ...
      if (text) { setForm(f => ({ ...f, text })); setRecStatus("✓ Записано"); showToast("Записано ✓"); }
      else setRecStatus("Не удалось распознать — попробуйте ещё раз");
    } catch {
      setRecStatus("Ошибка распознавания");
      showToast("Ошибка распознавания", "err");
    } finally {
    ...
    <div style={s.root}>
      ...
      {view==="add" && (
        <div style={s.card}>
          ...
          <div style={s.voiceRow}>
            <button style={{...s.micBtn,...(isRecording?s.micBtnActive:{}),...(isTranscribing?s.micBtnTranscribing:{})}}
              onPointerDown={!isTranscribing?startVoice:undefined} onPointerUp={!isTranscribing?stopVoice:undefined} onPointerLeave={!isTranscribing?stopVoice:undefined} disabled={isTranscribing}>
              {isTranscribing?"⏳":isRecording?"🔴":"🎤"}
            </button>
            <span style={s.recStatus}>{recStatus||"Удерживайте для записи"}</span>
          </div>
      ...
      {view==="settings" && (
        <div style={s.card}>
          ...
          <div style={{...s.keyStatus,...(apiKey?s.keyOk:s.keyNone)}}>{apiKey?"✓ API ключ подключён":"⚠ API ключ не задан — голос не работает"}</div>
          ...
          <hr style={s.sep}/>
          <div style={{...s.secTitle,fontSize:14,marginBottom:8}}>Где взять ключ?</div>
          <p style={s.hint}>1. Откройте <strong>console.anthropic.com</strong><br/>2. API Keys → ваш ключ MindFlow<br/>3. Скопируйте и вставьте выше</p>
...
const s = {
  ...
  micBtnTranscribing:{background:"#FFF8E8",opacity:0.8},
