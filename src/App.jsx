...
<span>{ico}</span><span style={{fontSize:11}}>{lbl}</span>
  ...
  {view==="settings" && (
    <div style={s.card}>
      ...
      <div style={{...s.keyStatus,...(apiKey?s.keyOk:s.keyNone)}}>{apiKey?"✓ API ключ подключён":"⚠️ API ключ не задан"}</div>
      ...
      {apiKey&&(<><hr style={s.sep}/><button style={s.dangerBtn} onClick={deleteApiKey}>🗑️ Удалить ключ</button></>)}
    </div>
  )}
